...
export const scanFolder = async (
  ...
  while (currentIndex < files.length) {
    ...
    batch.forEach((file) => {
      ...
      uploadPromises.push(
        uploadFile(fullFilePath, s3, 'movie-bucket', `${filename}/${file}`),
      );
    });
...
export const tsChunkProcessPipeLine = async (
  filePathBatch: string[],
  s3: AWS.S3,
  filename: string,
) => {
  try {
    ...
    console.error('Error processing ts chunk:', error);
  }
};

...
export async function registerMovieUploadTicketService(
  ...
  const isValidRegister = MovieDomainServices.validateDataCreateTicket(
    name,
    description,
    genres,
    releaseYear,
  ...
  // call from domain services
  const hashTicket = MovieDomainServices.hashTheTicket(
    name,
    description,
    releaseYear,
  );
  ...
  const writeTicketToRedis = await this.movieRepository.registerTicket(
    hashTicket,
    name,
    description,
    releaseYear,

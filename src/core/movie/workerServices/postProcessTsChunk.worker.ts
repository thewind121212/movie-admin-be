...
import {
  MOVIE_BUCKET,
  CALL_BACK_CHECK_INTERVAL,
  POST_PROCESSING_DELAY,
  TS_CHUNK_SNAPSHOT_DELAY,
} from '../movie.config';

export const checkQueueFinished = async (
  tsChunkProcessQueue: Queue,
  videoName: string,
): Promise<boolean> => {
  const jobs: Job[] = await tsChunkProcessQueue.getJobs(['waiting', 'active']);
  const isStillHaveJob = jobs.some((job) =>
    job.id?.toString().startsWith(videoName + '-'),
  );
  if (isStillHaveJob) {
    console.log('Queue is not finished yet', jobs.length);
    await new Promise((resolve) =>
      setTimeout(resolve, CALL_BACK_CHECK_INTERVAL),
    );
...
const checkjob = async (tsChunkProcessQueue: Queue, videoName: string) => {
  try {
    const jobs: Job[] = await tsChunkProcessQueue.getJobs([
      'waiting',
      'active',
      'delayed',
    ]);
    ...
    if (!isStillHaveJob) {
      ...
      await fs.promises.rm(path.resolve(`processed/${videoName}`), {
        recursive: true,
        force: true,
      });
      console.log(`Directory processed/${videoName} removed.`);
      void tsChunkProcessQueue.clean(0, 'completed');
      return {
        success: true,
...
export class tsChunkProcesser {
  ...
  async thumbnailRender(
    job: Job<{ videoName: string; tsChunkName: string; dirName: string }>,
  ) {
    const { videoName, tsChunkName } = job.data;
    await this.dockerServices.renderTSchunkThumnail(videoName, tsChunkName);
    await new Promise((resolve) =>
      setTimeout(() => resolve(true), TS_CHUNK_SNAPSHOT_DELAY),
    );
  }

  @Process('upload-snapshot')
  async uploadSnapShot(
    job: Job<{ snapShotPaths: string[]; videoName: string }>,
  ) {
    ...
    while (currentIndex < job.data.snapShotPaths.length) {
      const batch = job.data.snapShotPaths.slice(
        currentIndex,
  ...
  async handleVideoTranscoding(
    job: Job<{
      tsChunkBatchPaths: string[];
      videoName: string;
      batchType: 'normal' | 'rest';
    }>,
  ) {
    ...
    for (const tsChunkPath of tsChunkBatchPaths) {
      ...
      if (ext === '.m3u8') {
        ...
        const modifiedM3u8Content = m3u8Content.replaceAll(
          'segment',
          `${process.env.S3_SERVICE_ENDPOINT}/${MOVIE_BUCKET}/${videoName}/segment`,
        );
        ...
        chokidar
          ...
          .once('add', () => {
            ...
            void this.tsChunkProcessQueue.add(
              'gen-snapshot',
              { videoName, tsChunkName: baseName, dirName },
              {
    ...
    await new Promise((resolve) =>
      setTimeout(() => resolve(true), POST_PROCESSING_DELAY),
    );

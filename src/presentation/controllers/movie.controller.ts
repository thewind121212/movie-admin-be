...
export class MovieController {
  ...
  async uploadMovie(@Body() body: {
    name: string;
    description: string
  }, @Req() req: ExpressRequest, @Response() res: ExpressResponse) {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      await this.S3services.upLoadToS3(RAW_MOVIE_BUCKET, movileName, fileStream);
      console.log(uploadTicket)
      const uploadResult = await this.movieServices.uploadMovie(`${process.env.S3_SERVICE_ENDPOINT}/${RAW_MOVIE_BUCKET}/${movileName}`, 'video', uploadTicket as string)
